...
2. **Aura 的 Python actor**。actor 产出 okm 操作、发送到 Aura 对嵌套存储执行。actor 的语言是部署事实，不是互操作需求。
...
- **远程模式（Aura：actor 本地执行，远程机械执行）**：callable 在 actor 自己的运行时里跑；操作载荷携带语义「结果」——派生好的条目字节、绝对累加器值（「acc 变为 42」）、以及覆盖 unfold 所需的旧文档（或其版本）。远程把它们作为一帧批量执行（ADR-0010 §2 的单次 `commit_batch`），文档写与累加器更新保持原子，而远程对两者都不做语义理解。远程不宿主 callable、不解析语义——ADR-0010 的接收方契约原样成立。
...
- **每 group 单写者是远程 reduce 模式的前提。** Aura 的分区 actor 模型按构造满足（actor 拥有它写的数据）。同一规则下 actor 可以本地缓存累加器——它就是权威值——稳态 put 携带绝对 acc、零额外往返；只有重启恢复时读一次 acc（与 Redis 式共享 RMW 模式的显式对照：后者需要 CAS/事务恰恰因为它多写者）。
- 多写者 group 不适用此模式。其语义执行属于拥有者进程（Rust 侧 derive，或专职的拥有者 actor）。这是范围边界，不是要被工程化消除的缺陷。
...
2. 远程模式：Python actor 产出的操作在远端引擎执行后，与 Rust 侧产出的同一操作落得字节一致（现有跨语言字节相等测试，从 codec 字节扩展到语义条目）。
...
- 「暂不、非永不」：subscribe 对齐。若宿主语言的消费者故事出现（actor 要在进程内消费 reduce 组事件），以通道协议为设计面重新开启。
