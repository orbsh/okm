...
- **GROUP name resolution becomes two-source.** `#[ok_reduce(Logic { group(a, b) })]` may name key and payload fields. Byte compatibility: the group segment's layout rule is unchanged — declared names in order, each field's fixed-width BE encoding; the ENCODER per name is the only difference. *Amendment (2026-09-23, implementation): the original compile-error rule for a name present in both sources was demoted to KEY-WINS resolution — the key struct is derived by a separate macro expansion, so the DocumentEncode derive cannot see key field names at compile time. Resolution lives in the generated `__okm_encode_group_named` walk (and the dynamic side's `group_bytes`, same rule).*
