...
Date: 2026-09-06
Status: Superseded at the macro level by [ADR-0006](0006-row-node-model.md) (the standalone `ValueEncode` derive is cancelled; its encoding rules live on as the payload half of `ObjEncode`). The mechanisms below remain authoritative.
