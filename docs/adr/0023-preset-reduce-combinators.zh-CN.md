...
- **key 侧量的镜像字段问题仍在。** reduce 折叠的是 payload 字段；当被聚合的量在 key 里（aura 的 proxy id），payload 必须带镜像。组合子不改变这一点——`Max<F>` 聚合 payload 字段，与任何用户 logic 一样。「对 key 字段做 reduce」的扩展是独立想法、另有 ADR，不在这里夹带。
