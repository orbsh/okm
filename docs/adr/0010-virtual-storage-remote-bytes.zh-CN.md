...
**裸分片形态。** 前缀是可选的（`StorageHost::bare`）：裸宿主按字节原样执行帧——发送方的键空间就是引擎的键空间。它服务分片：同一业务领域的 N 个分片（每分片部署同一份编译产物、同一 ns 字典、同一编码）各持一个裸宿主；分片与路由归编排层（如 Aura 的 partition-key 路由），OKM 零检查、零机制。前提是指向该宿主的所有分片实例域建模一致——由部署保证，运行期不可判定（判定它就是宿主不该有的 OKM 语义）。裸宿主与托管宿主可共居一个引擎：托管段之间 2 字节互斥，编排层的分配职责是让裸实例的 ns 编号避开托管段号——这是持有全局视图的那一层的分配职责，不是宿主里的运行期检查。
