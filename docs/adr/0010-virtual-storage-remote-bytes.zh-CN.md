...
- **线上没有版本头。** 布局版本化是发送方的进程内事务（硬编码 hex 稳定性测试 + `#[ok_layout(version)]` 解码拒绝）。Aura 不运行发送方的 OKM，不持有布局知识；两端唯一的契约是 op 字节 + 字节流。（此条推翻了早期草案帧头放布局版本的设计——错在把两个刻意无关的端语义耦合了。）
- **线上不用通用序列化库。** 帧是「数出来的字段」，不是协议：op 已在 trait 边界编码完毕，帧只需要标出 op 边界——tag + 长度 + 裸字节，手写解析（约 30 行含边界检查）。serde/postcard 换来的是「帮我们数长度」，代价是两个依赖加一套 trait 机制，而收益为零：体积逐字节相同（varint 长度前缀就是同样的字节），解析同量级（每 op 几条指令 vs WAL commit 的微秒——测不出来），还多出一层边界不该有的语义层（与上条拒绝版本头是同一形状的理由）。value 压缩若有，属于字段 wrapper 层（VarInt/Quant——值语义在那里才知道），永不属于帧；帧看到的压缩字节只是「更短的字节」。
- **帧布局**（手写解析，无总长头——传输层（`Vec<u8>` 消息 / TCP 分帧）已定界，帧内重述是冗余）：

  ```text
  写帧: [op_count 定档编码]
        每 op: [tag+长度档 u8][key 长度 LK][value 长度 LV][key][value]
      tag nibble: put / delete / get / scan（4 形态，2 位，保留 2 位）
      长度编码（LK、LV 同一格式），首字节：
        00xxxxxx                              内联 ≤63
        01xxxxxx + 1 字节                      14 位
        10xxxxxx + 2 字节                      22 位
        11xxxxxx + 4 字节                      32 位
      （首字节低 6 位是值的高位；四个宽度档吸收了「极小内联」——4 位长度
      档永远命中不了：真实 key = ns + payload，几十字节起步）
  ```

  位移/掩码解码是单周期流水线指令——位打包的真实代价是每字段一个 selector 分支、坏帧测试面变宽（维护性，不是速度）。超限长度（超过剩余字节）拒绝，永不 panic。
...
### 4. 接收方声明：`NestStorage` derive

接收方是声明出来的，不是手接的。空结构体标注 `NestStorage` (with `#[ok_ns]`) 只声明前缀（服务哪个应用/前缀）。derive 生成**无数据方法**（无 put/get/scan——没有可编码的行类型），只有一个执行方法（`apply`）：收帧 → 前置声明前缀 → 普通引擎执行 → 返回响应帧（get/scan 回 Some，put/delete 回 None——返回值本身就是"有没有响应"的答案）。

一次钉死三件事：前缀来源（声明，不是散落配置）、执行器形状（宏生成，不是手拼）、前缀逃逸的结构不可能（句柄构造时绑定前缀；跨命名空间不可表达——物理分隔，不是命名过滤）。与 `#[ok_subscribe]` 同一纪律：注解声明事实，宏生成实现。

### 5. 多租户：接收方前缀，一个实例一个 ns

多租户的数据库级机制只有一种：接收方声明的前缀。一个远程 OKM 实例 = 一个应用 = 一个领域模型 = 自己的 ns 字典；对接收方它只是又一个前缀。OKM 内部没有 `app_id` 层，无保留前缀值，无多级 ns 声明——应用内部要按租户分片，`tenant_id` 是它结构体里的普通 key 字段（业务分片，所有租户同一建模）；只有当整个应用在平台层隔离时，接收方才为每个应用托管一个 `NestStorage` (with `#[ok_ns]`) 执行器，各自声明前缀，各自后面挂自己的单级 ns 字典。

**裸分片形态。** 前缀是可选的（`NestStorage::bare`）：裸宿主按字节原样执行帧——发送方的键空间就是引擎的键空间。它服务分片：同一业务领域的 N 个分片（每分片部署同一份编译产物、同一 ns 字典、同一编码）各持一个裸宿主；分片与路由归编排层（如 Aura 的 partition-key 路由），OKM 零检查、零机制。前提是指向该宿主的所有分片实例域建模一致——由部署保证，运行期不可判定（判定它就是宿主不该有的 OKM 语义）。裸宿主与托管宿主可共居一个引擎：托管段之间 2 字节互斥，且托管的应用自身也可分片（一个声明应用的多个实例，各自持裸宿主，按用户/分区键路由）。唯一的碰撞面是字节巧合——裸实例的 ns 表字节（`[0, N]...`）撞上托管段前缀 `[0, M]`（N = M 时）——编排层的规避方式是分配托管段号时避开同引擎裸分片的域内 ns 号。这是持有全局视图的那一层的分配职责，不是宿主里的运行期检查（裸宿主不知道托管段的存在，也不该知道）。
...
接收方只知道自己的前缀；ns 段是发送方的——接收方前置前缀后从不去解析后面是什么；ns 字节不透明地穿过。知识不对称即隔离机制：接收方无法越过看不见的东西路由，发送方无法逃出自己不持有的前缀（`NestStorage` (with `#[ok_ns]`) 句柄构造时绑定前缀）。ADR-0002 的「若租户存在」条款由此**从草图提升为正式机制**；它当初否决租户外侧隔离的前提是「无用户可编程查询面」，dynamic 执行模式改变了这个前提——本 ADR 即修订触发器。ns 字典纪律本身（编译期、append-only、u16）不动。
...
对称地，接收方也不知道到达路径。`NestStorage` (with `#[ok_ns]`) 执行器的面恰好一个方法——帧进，结果出；谁调它、走什么通道是调用方的事。读关联（哪个响应回答哪个请求）在执行器的发送侧，同写侧。TCP + 手写解析客户端是参考示例，不是契约的一部分。传输多样性只存在于执行器两侧的外部，永不漏进执行器。
