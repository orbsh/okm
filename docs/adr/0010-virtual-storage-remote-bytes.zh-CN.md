...
接收方是声明出来的，不是手接的。空结构体标注 `#[kv_storage(...)]` 只声明前缀（服务哪个应用/前缀）。derive 生成**无数据方法**（无 put/get/scan——没有可编码的行类型），只有一个执行方法（`apply`）：收帧 → 前置声明前缀 → 普通引擎执行 → 返回响应帧（get/scan 回 Some，put/delete 回 None——返回值本身就是"有没有响应"的答案）。
...
**裸分片形态。** 前缀是可选的（`StorageHost::bare`）：裸宿主按字节原样执行帧——发送方的键空间就是引擎的键空间。它服务分片：同一业务领域的 N 个分片（每分片部署同一份编译产物、同一 ns 字典、同一编码）各持一个裸宿主；分片与路由归编排层（如 Aura 的 partition-key 路由），OKM 零检查、零机制。前提是指向该宿主的所有分片实例域建模一致——由部署保证，运行期不可判定（判定它就是宿主不该有的 OKM 语义）。裸宿主与托管宿主可共居一个引擎：托管段之间 2 字节互斥，且托管的应用自身也可分片（一个声明应用的多个实例，各自持裸宿主，按用户/分区键路由）。唯一的碰撞面是字节巧合——裸实例的 ns 表字节（`[0, N]...`）撞上托管段前缀 `[0, M]`（N = M 时）——编排层的规避方式是分配托管段号时避开同引擎裸分片的域内 ns 号。这是持有全局视图的那一层的分配职责，不是宿主里的运行期检查（裸宿主不知道托管段的存在，也不该知道）。
