...
//!
//! Document types + the generated module live in subscribe_common.rs (shared
//! with upsert_test — the generated enum's `super::` references must
//! resolve in every binary that includes it).

use okm_core::{TestStore, Collection};
...
fn subscribe_round_trip() {
    ...
    let mut t: Collection<TestStore, AccountKey, Account> = Collection::new(TestStore::slatedb_mem());
...
fn no_sink_drops_silently() {
    ...
    let mut t: Collection<TestStore, GhostKey, Ghost> = Collection::new(TestStore::slatedb_mem());
