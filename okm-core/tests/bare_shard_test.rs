...
use okm_core::{KeyEncode, MockStore, RemoteStore, Row, RowEncode, StorageHost, Table, VirtualStorage};

#[test]
fn bare_host_executes_frames_byte_identical() {
    let handle = StorageHost::bare(MockStore::default());
...
fn bare_and_hosted_coexist_with_allocation_discipline() {
    ...
    let engine = MockStore::default(); // handle-clone = shared engine
