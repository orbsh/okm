...
fn varint_frame_is_variable_length() {
    ...
    // hits=1000 → 2 bytes; cold frame = 1 tag + 1 len + 2 val at 16.
    ...
    assert_eq!(p[17], 2); // len varint = 2
    assert_eq!(&p[18..20], &[0x83, 0xE8]); // varint(1000): w=2, byte order = value order
}
