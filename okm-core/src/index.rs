...
pub trait Document: Sized + Clone {
    /// Embedded-field dereference (ADR-0012 embedded milestone): after the
    /// parent payload decodes, fetch each `Embedded` field's child document
    ...
    /// Embedded-field write entries: `(child pkey, child payload)` pairs
    /// for fields carrying `value: Some(_)`. Default empty.
    fn __okm_embed_entries(&self) -> Vec<(Vec<u8>, Vec<u8>)> {
        Vec::new()
    }
    /// Embedded-field keys previously pointed at (from a decoded OLD
