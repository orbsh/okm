//! Build script — collect every `#[ok_subscribe(Enum::Variant)]` row in
...
fn key_type_of(s: &syn::ItemStruct) -> Option<String> {
    s.attrs
        .iter()
        .filter(|a| a.path().is_ident("ok_ref"))
...
fn scan_file(path: &Path, acc: &mut Vec<(String, String, String, String)>) {
    ...
    for item in &ast.items {
        ...
        for a in &s.attrs {
            if a.path().is_ident("ok_subscribe") {
                ...
                if a.parse_args::<syn::ExprPath>().is_ok() {
                    panic!(
                        "ok_subscribe[{}]: variant paths are not supported — declare \
                         `#[ok_subscribe]` (bare); the variant is the row type name",
                        s.ident
                    );
                }
                sub = true;
            }
            if a.path().is_ident("ok_event_enum") {
                let body = a
                    ...
                    .unwrap_or_else(|| {
                        panic!(
                            "ok_event_enum[{}]: expected an enum name, e.g. `#[ok_event_enum(MyEvents)]`",
                            s.ident
                        )
                    });
                if body.split("::").count() != 1 {
                    panic!("ok_event_enum[{}]: expected a bare enum name, got `{body}`", s.ident);
                }
                enum_name = Some(body);
            }
        }
        if sub {
            let key_ty = key_type_of(s).unwrap_or_else(|| {
                panic!("ok_subscribe[{}]: no `#[ok_ref(KeyTy)]` found", s.ident)
            });
...
fn main() {
    ...
    let mut code = String::from(
        "// Generated by build.rs — DO NOT EDIT.\n\
         // Event enum(s) + channel cells collected from `#[ok_subscribe]` rows.\n\
